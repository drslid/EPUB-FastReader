...
describe("disponibilité des sources", () => {
  it("vérifie les vrais endpoints sans télécharger un EPUB, puis réutilise le contrôle pendant cinq minutes", async () => {
    const fetchImpl = vi.fn(async (url) => new Response(null, { headers: { "Content-Type": url.includes("ebooksgratuits") ? "application/atom+xml" : "application/epub+zip" } }));
  ...
  it("ne transforme pas une page HTML de blocage ou un timeout en pastille verte", async () => {
    vi.useFakeTimers();
    const fetchImpl = vi.fn((url) => url.includes("ebooksgratuits") ? new Promise(() => {}) : Promise.resolve(new Response("blocked", { headers: { "Content-Type": "text/html" } })));

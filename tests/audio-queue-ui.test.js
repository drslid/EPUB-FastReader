...
let queue, ui, snapshot, onListen, onBrowse, onManageStorage, changed, originalScrollIntoView;
...
beforeEach(() => {
  ...
  ui = createAudioQueueUI({ queue, onListen, onBrowse, onManageStorage });
});
afterEach(() => {
...
describe("local audio queue dialog", () => {
  ...
  it("reserves the full sticky header inset including dialog padding and fractional pixels", async () => {
    ...
    const geometry = vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(function () {
      if (this.classList.contains("audio-queue-heading")) return { top: 31.5, bottom, height: bottom - 31.5 };
      if (this.classList.contains("audio-queue-dialog")) return { top, bottom: 628, height: 628 - top };
    ...
    try {
      ...
      expect(dialog.style.getPropertyValue("--audio-queue-header-inset")).toBe("99px");
      top = 8.25; bottom = 126.6;
      window.dispatchEvent(new Event("resize"));
      expect(dialog.style.getPropertyValue("--audio-queue-header-inset")).toBe("119px");
    } finally {
      geometry.mockRestore();
    }
  });

  it("keeps the central storage entry stable while preparation progresses", async () => {
    snapshot.jobs = [makeJob("ready", "ready"), { ...makeJob("pending", "queued"), audioBytes: 0 }];
    ui.open(); await settled();
    const storage = document.querySelector("[data-audio-queue-storage]");
    storage.focus();
    snapshot.jobs[1].audioBytes = 2_000_000;
    changed(snapshot);
    expect(document.querySelector('[data-audio-job="pending"]').textContent).toContain("2 Mo");
    expect(document.querySelector("[data-audio-queue-storage]")).toBe(storage);
    expect(document.activeElement).toBe(storage);
    expect(document.querySelector("[data-audio-queue-clear]")).toBeNull();
    expect(queue.clearAll).not.toHaveBeenCalled();
  });

  it("opens storage for the current book and closes the queue without deleting anything", async () => {
    snapshot.jobs = [makeJob("ready", "ready")];
    ui.open({ bookId: "ready" }); await settled();
    const storage = document.querySelector("[data-audio-queue-storage]");
    storage.click();
    expect(onManageStorage).toHaveBeenCalledExactlyOnceWith({ bookId: "ready" });
    expect(document.querySelector("dialog")).toBeNull();
    expect(document.activeElement).toBe(document.querySelector("#queue-trigger"));
    expect(queue.clearAll).not.toHaveBeenCalled();
    expect(queue.remove).not.toHaveBeenCalled();
    expect(queue.cancel).not.toHaveBeenCalled();
  });

  it("keeps storage available without jobs so downloaded voices can still be managed", async () => {
    ui.open(); await settled();
    const storage = document.querySelector("[data-audio-queue-storage]");
    expect(storage.disabled).toBe(false);
    storage.click();
    expect(onManageStorage).toHaveBeenCalledExactlyOnceWith({ bookId: "" });
    expect(queue.clearAll).not.toHaveBeenCalled();
  });

  it("localizes the central storage entry in every interface language without moving focus", async () => {
    snapshot.jobs = [makeJob("ready", "ready")];
    ui.open(); await settled();
    const storage = document.querySelector("[data-audio-queue-storage]");
    storage.focus();
    for (const language of ["fr", "en", "es", "it", "de", "pt"]) {
      ...
      expect(storage.textContent).toContain(t("Gérer le stockage audio"));
      expect(document.querySelector("[data-audio-queue-storage]")).toBe(storage);
      expect(document.activeElement).toBe(storage);
    }
  });

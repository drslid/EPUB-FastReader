...
import voice from "../src/locales/voice.js";
import audioQueue from "../src/locales/audio-queue.js";
...
describe("translated reading interface", () => {
  it("provides every view key in each translation with intact placeholders", () => {
    ...
    for (const language of ["en", "es", "it", "de", "pt"]) {
      const dictionary = { ...views[language], ...sources[language], ...sourceSettings[language], ...loyalSearch[language], ...voice[language], ...audioQueue[language] };

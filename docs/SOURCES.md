...
- **Atramenta retiré** : l’essai depuis le relais Cloudflare public a reçu un refus (`503 SOURCE_BUSY`). La source, ses routes et ses contrôles de disponibilité sont supprimés. Les livres déjà importés restent lisibles dans la bibliothèque locale. [Audit historique](SOURCE-ATRAMENTA-AUDIT.md).
- **Loyal Books** : recherche en ligne dans un panneau Google distinct, commandé par la barre commune. Les titres anglais sont inclus. Les résultats, liens, annonces éventuelles, attribution et pagination Google restent intacts. L’action Lire vérifie la fiche EPUB via le relais puis utilise l’import local habituel. La sélection locale de 499 notices et son collecteur sont supprimés. [Audit](SOURCE-LOYALBOOKS-AUDIT.md).
...
Loyal Books déclare `searchPresentation: "embedded"` : il est sélectionnable mais exclu de la fusion « Tout le catalogue ». Son composant Google documenté n’est chargé qu’après sélection explicite et saisie d’une recherche. Aucun moteur Google interne, jeton réutilisé ou index FastReader ne fournit ces résultats. Les anciens livres Loyal Books restent dans IndexedDB et se rouvrent sans téléchargement supplémentaire.

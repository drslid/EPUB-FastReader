...
export function searchBarMarkup(state, { icon, escape }) {
  ...
  const loyalSearch = state.provider === "loyalbooks" && ["search", "discover"].includes(state.view);
  return `<form id="search-form" class="unified-search" role="search" aria-label="${t("Rechercher des livres")}">
    <div class="unified-search-field">${icon("search")}<label class="sr-only" for="search-query">${t("Titre ou auteur")}</label><input id="search-query" name="query" type="search" maxlength="150" placeholder="${t(loyalSearch ? "Titre ou auteur sur Loyal Books…" : "Mes livres et les catalogues…")}" value="${escape(query)}" autocomplete="off" enterkeyhint="search"><button type="button" class="unified-search-clear" data-action="clear-search" aria-label="${t("Effacer la recherche")}" ${query || state.query ? "" : "hidden"}>${icon("close")}</button></div>
    ${loyalSearch ? `<input type="hidden" name="language" value="">` : `<div class="unified-search-language"><label class="sr-only" for="search-language">${t("Langue du livre")}</label><span class="unified-language-code" aria-hidden="true">${language.value ? language.short : t("Tous")}<span>⌄</span></span><select id="search-language" name="language">${languages.map(({ value, label }) => `<option value="${value}" ${language.value === value ? "selected" : ""}>${value ? label : t(label)}</option>`).join("")}</select></div>`}

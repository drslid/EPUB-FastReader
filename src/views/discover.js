  ...
  const renderBook = (book, index) => {
    ...
    const availability = inLibrary
      ...
      : needsConnection && state.offline
        ? "Connexion requise"
        : "";
    return `<article class="book-card ${inLibrary ? "is-in-library" : ""}" data-provider="${escape(book.providerId)}" data-language="${escape(book.language || "")}"><button class="book-open" data-action="catalog-read" data-id="${escape(book.id)}" aria-label="Lire ${escape(book.title)}" ${state.busy ? "disabled" : ""}>${cover(inLibrary ? { ...book, downloadMode: "local" } : book, index)}<h3>${escape(book.title)}</h3><p>${escape(book.author)}</p></button>
      ...
      <div class="book-card-meta">${availability ? `<span class="availability is-ready">${icon(needsConnection && state.offline ? "download" : "check")} ${availability}</span>` : ""}<a href="${escape(book.sourceUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Source et droits : ${escape(book.title)}">Source ${icon("external")}</a></div>
      <div class="catalog-book-actions"><button class="button secondary catalog-download-button" data-action="catalog-download" data-id="${escape(book.id)}" aria-label="Obtenir l’EPUB : ${escape(book.title)}" ${state.busy ? "disabled" : ""}>${icon("download")} Obtenir l’EPUB</button><button class="button ink start-book-button" data-action="catalog-read" data-id="${escape(book.id)}" ${state.busy ? "disabled" : ""}>${inLibrary ? "Reprendre" : "Lire"} ${icon("play")}</button></div></article>`;
  };

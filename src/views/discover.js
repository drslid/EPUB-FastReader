...
export function discoverMarkup(state, { icon, escape, cover, providers }) {
  ...
  const renderBook = (book, index) => {
    ...
    return `<article class="book-card ${inLibrary ? "is-in-library" : ""}" data-provider="${escape(book.providerId)}" data-language="${escape(book.language || "")}"><button class="book-open" data-action="catalog-read" data-id="${escape(book.id)}" aria-label="${t("Lire {title}", { title: escape(book.title) })}" ${state.busy ? "disabled" : ""}>${cover(inLibrary ? { ...book, downloadMode: "local" } : book, index)}<h3>${escape(book.title)}</h3><p>${escape(book.author)}</p></button>
      ...
      <div class="book-card-meta">${availability ? `<span class="availability is-ready">${icon(needsConnection && state.offline ? "download" : "check")} ${availability}</span>` : ""}<a href="${escape(book.sourceUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${t("Source et droits : {title}", { title: escape(book.title) })}">${t("Source")} ${icon("external")}</a></div>
  ...
  return `${unified ? "" : `<section class="discovery-intro"><div class="eyebrow">${t("DES LIVRES LIBRES À EXPLORER")}</div><h1>${t("Une envie de <em>lecture ?</em>")}</h1><p>${t("Touchez une couverture pour commencer votre prochaine histoire.")}</p></section>`}
    ...
    ${(state.catalogWarnings || []).map((warning) => `<p class="source-warning" role="status">${escape(warning.message)} ${t("Les livres intégrés restent accessibles.")}</p>`).join("")}
    ...
    ${unified ? `<p class="catalog-download-hint">${t("La langue et les sources filtrent uniquement les catalogues. Les éditions déjà enregistrées sont affichées dans « Mes livres ». Un premier téléchargement nécessite une connexion ; la lecture reste ensuite disponible hors ligne.")}</p>` : ""}
    ...
    <p class="rights-note">${t("Les éditions intégrées conservent leur provenance et leur licence. Gutenberg indique les droits aux États-Unis ; vérifiez ceux de votre pays.")} <a href="${import.meta.env.BASE_URL}books/NOTICE.html" target="_blank" rel="noopener noreferrer">${t("Sources et droits des livres intégrés")}</a> · <a href="https://publicdomainlibrary.org/en/ebooks" target="_blank" rel="noopener noreferrer">${t("Autres livres sur Public Domain Library")} ${icon("external")}</a></p>`;
}

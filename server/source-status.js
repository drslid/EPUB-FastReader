  ...
  async function probe(definition) {
    ...
    try {
      const response = await Promise.race([
        fetchImpl(definition.url, { method: definition.method, redirect: "manual", credentials: "omit", signal: controller.signal, headers: { Accept: definition.method === "HEAD" ? "application/epub+zip" : "application/atom+xml" } }),
      ...
      const available = response.status === 200 && !response.redirected && (!response.url || response.url === definition.url) && (definition.contentType === "xml" ? /(?:atom\+xml|application\/xml|text\/xml)/iu.test(type) : /(?:epub\+zip|octet-stream|application\/zip)/iu.test(type));
